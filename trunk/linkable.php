...
if (! class_exists('Void\Linkable')) {
    class Linkable
    {
        ...
        protected function getPostYoastTitle(int $postId): string
        {
            ...
            $cacheKey = 'yoast_title_' . $postId;
            $cached = wp_cache_get($cacheKey, 'linkable');

            if ($cached !== false) {
                return $cached;
            }

            // Sanitize table name (minimal risk)
            $tableSafe = esc_sql($table);

            $title = $wpdb->get_var(
                $wpdb->prepare("SELECT title FROM `$tableSafe` WHERE object_id = %d LIMIT 1", $postId)
            );

            if (!$title) {
                $title = get_the_title($postId);
            } else {
                $title = trim(str_replace(['%%sep%%', '%%sitename%%', '%%page%%'], '', $title));
            }

            wp_cache_set($cacheKey, $title, 'linkable', HOUR_IN_SECONDS);
        ...
        {
            return [
                'max_links_per_target' => absint($input['max_links_per_target'] ?? 1),
                'first_occurrence_only' => !empty($input['first_occurrence_only']) ? 1 : 0,
            ];
        }
            ...
            add_settings_field('first_occurrence_only', 'Only link first keyword match?', function () {
                ...
                <label>
                    <input type="checkbox"
                           ...
                           value="1" <?php checked(1, $value, false) ?> />
